...
| 버전 | 일시 | 내용 | 비고 |
| :--- | :--- | :--- | :--- |
| v4.0.2 | 2026-05-29 16:15:00 | [Core/QA] 코드 전수 검증 기반 7개 핵심 버그 수정: 1) trader.py — recently_entered TTL 120초→180초 (LIMIT_ORDER_TIMEOUT 180초와 일치), 2) trader.py — on_signal 진입 시 실시간 포지션 청산 감지 루프 추가 및 즉시 쿨다운(60초) 가동으로 채터링 방지 복원, 3) trader.py — 동일 캔들 중복 진입 방지 가드(last_entered_candle_ts) 복원, 4) exchange.py — Binance 시장가 주문 False Cancel 버그 수정(최소 1회 폴링 강제로 API 전파 지연 대응), 5) strategy.py — 스퀴즈 확장 윈도우 lookback*3(24봉)→lookback+4(12봉) 수정으로 과신호 방지, 6) harness.py — OKX_API_KEY 환경변수 참조→BINANCE_API_KEY로 수정하여 진단 도구 정상화, 7) engine.py — 텔레그램 알림 내 OKX 브랜드 텍스트→Binance 수정. 전체 123개 테스트 100% 통과 | Core/QA |
