...
| 버전 | 일시 | 내용 | 비고 |
| :--- | :--- | :--- | :--- |
| v2.4.4 | 2026-05-28 01:15:00 | [Config/UI/Test] 1) core/config.py 내 트레일링 스탑 설정(TRAILING_ACTIVATE_PCT, TRAILING_CALLBACK_PCT)을 정식 필드로 등록, 2) 사이드바 및 설정 탭에 트레일링 발동/콜백 조작 위젯 추가 및 .env 실시간 저장 연동, 3) tests/test_strategy.py 내 pandas(pd) 임포트 누락으로 인한 NameError 수정 | Config/UI/Test |
