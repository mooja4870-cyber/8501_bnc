...
class TestRiskGateParameters:
    ...
    def test_balance_minimum(self):
        ...
        ok, reason = asyncio.run(self.trader._risk_check(_make_signal()))
        assert ok is False
    ...
    def test_signal_strength_minimum(self):
        ...
        import asyncio
        ok, reason = asyncio.run(self.trader._risk_check(_make_signal(strength=80)))
        assert ok is False
    ...
    def test_signal_strength_100_pass(self):
        """신호 강도 = 100% → 통과"""
        import asyncio
        ok, reason = asyncio.run(self.trader._risk_check(_make_signal(strength=100)))
        assert ok is True
...
class TestPositionGuardParameters:
    ...
    def test_duplicate_position_blocked(self):
        ...
        asyncio.run(self.trader.on_signal(_make_signal(symbol="BTC/USDT:USDT")))
    ...
    def test_max_positions_blocked(self):
        ...
        before = self.trader.orders_today
        asyncio.run(self.trader.on_signal(_make_signal(symbol="NEW/USDT:USDT")))
        assert self.trader.orders_today == before
    ...
    def test_recently_entered_guard(self):
        ...
        asyncio.run(self.trader.on_signal(sig))
    ...
    def test_long_direction_blocked(self):
        ...
        before = self.trader.orders_today
        asyncio.run(self.trader.on_signal(_make_signal(direction="long")))
        assert self.trader.orders_today == before
    ...
    def test_short_direction_blocked(self):
        ...
        before = self.trader.orders_today
        asyncio.run(self.trader.on_signal(_make_signal(direction="short")))
        assert self.trader.orders_today == before
    ...
    def test_disabled_trader_blocked(self):
        ...
        self.trader.disable()
        before = self.trader.orders_today
        asyncio.run(self.trader.on_signal(_make_signal()))
        assert self.trader.orders_today == before
    ...
    def test_none_signal_ignored(self):
        ...
        import asyncio
        before = self.trader.orders_today
        asyncio.run(self.trader.on_signal(_make_signal(direction="none")))
        assert self.trader.orders_today == before
...
class TestMaxPositionsRaceCondition:
    ...
    def test_sequential_signals_respect_max(self):
        ...
        for sym in symbols:
            asyncio.run(self.trader.on_signal(_make_signal(symbol=sym)))

        assert self.trader.orders_today == CFG.MAX_POSITIONS
        assert len(asyncio.run(self.mock.get_positions())) == CFG.MAX_POSITIONS
