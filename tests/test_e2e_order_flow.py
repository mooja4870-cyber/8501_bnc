...
@pytest.mark.asyncio
async def test_full_order_flow_e2e():
    ...
    # 초기 잔고 상태 기록
    initial_balance = await mock.get_balance()
    ...
    # 3. 리스크 게이트 검증
    ok, reason = await trader._risk_check(long_signal)
    assert ok is True
    assert reason == "OK"
    ...
    # 잔고 업데이트 검증
    balance_after = await mock.get_balance()
    ...
    trades = await mock.get_trade_history()
    assert len(trades) > 0
    ...
    success = await mock.close_position("BTC/USDT:USDT", "long")
    assert success is True
    
    # 청산 완료 후 잔고 상태 최종 검증
    final_balance = await mock.get_balance()
    assert len(await mock.get_positions()) == 0
