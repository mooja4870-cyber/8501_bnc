...
`.env` 파일을 열어 Binance API 키를 입력하세요:

```
BINANCE_API_KEY=실제_API_키
BINANCE_SECRET_KEY=실제_시크릿_키
BINANCE_PASSPHRASE=실제_패스프레이즈_혹은_공백
```

> **Binance API 키 발급 방법**
> Binance → API Management → API 생성
> 권한: Enable Futures ✅ (선물 거래 권한 체크 필수)
> IP 화이트리스트 설정 권장
...
- Stop-Loss는 자동 설정되나 급격한 변동성에는 100% 보호 불가
