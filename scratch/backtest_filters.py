...
def simulate_backtest(df_dict, filter_type=None):
    """
    Simulate trades on the last 192 candles (48 hours) for each ticker
    """
    total_trades = 0
    wins = 0
    total_pnl = 0.0
    ...
    for symbol, df in df_dict.items():
        ...
        for i in range(len(test_df)):
            ...
            if filter_type == 'ema200':
                ...
                # Filter 2: Strict MACD location (dot switch from opposite side of zero)
...
def main():
    ...
    print(f"Fetching 15m candles (400 bars) for top {len(top_symbols)} symbols...")
    df_dict = {}
    
    start_time = time.time()
    for idx, sym in enumerate(top_symbols):
        # Fetch 400 candles (to ensure 200 EMA is fully warmed up)
        df = client.get_ohlcv(sym, timeframe="15m", limit=400)
    ...
    print("\n" + "="*80)
    print("최근 48시간 백테스트 결과 비교 (타임프레임: 15m, 종목수: 80개)")
